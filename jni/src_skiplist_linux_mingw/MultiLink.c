		...
		if (that->map[i] & MAP_MASK) {
			for (j = 0; j < MAP_SHIFT && index < that->size; j++, index++) {
				if (that->map[i] & (0x01 << j)) {
					that->map[i] &= ~(0x01 << j);
					MultiLinkElement * o = that->at(that, index);
					o->index = index;
					return o;
				}
			}
...
void ElementPool_back(ElementPool * that, MultiLinkElement * o){
	...
	//you should make sure you're getting this o
	//from pool, so that the index is correct
	//otherwise index is an random number
	//because there is no way to initialize
	//index in _MultiLinkElement
	if (o->index > 0) {
		index = o->index;
		i = index / MAP_SHIFT;
		j = index - i * MAP_SHIFT;
